...
class AADSSO_Profile {
	...
	public function aad_sso_merge_button() {
		...
		if ( get_user_meta( $user_id, '_aad_sso_id', 1 ) ) {
			return;
		}
	...
	public function maybe_cache_user( $user ) {
		...
		if (
			...
			&& ! get_user_meta( $user_id, '_aad_sso_id', 1 )
	...
	public function maybe_connect_user( $user ) {
		if (
			$this->user_to_keep
			&& is_a( $user, 'WP_User' )
			&& ( $aad_sso_id = get_user_meta( $user->ID, '_aad_sso_id', 1 ) )
		) {
	...
	public function connect_accounts( $user_to_link ) {
		...
		update_user_meta( $this->user_to_keep, '_aad_sso_id', $user_to_link->aad_sso_id );
